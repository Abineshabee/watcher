...
for fn in [step_mem_off, step_mem_peak, step_mem_rss,
           step_mem_bool_false, step_mem_bool_true]:
    fn(df_base)
...
print(f"WatcherWarning fired: {any(issubclass(w.category, WatcherWarning) for w in caught)}")
...
be = BackendError("null_counts failed", backend_name="pandas", original=ValueError("oops"))
print(f"\nBackendError:\n{be}")
